...
import s from './OrderFail.scss';
import t from './translation.json';

// import CompleteOrder from '../../../components/CompleteOrder';
import Container from '../../../components/Container';
import { fetchOrderByIncrementId } from '../../../reducers/order/actions';
import OrderFailHeader from '../../../components/Order/OrderFailHeader';
...
const paymentUseRepayment = ['fullpaymentredirect', 'p2c2p_ipp'];
...
class OrderFail extends React.PureComponent {
  ...
  orderItemsMapping = async () => {
    ...
    const orderItems = map(order.items, item => ({
      ...
      productDetail: {
        ...
        base_image_url: this.props.activeConfig.base_media_url,
      },
    }));
  ...
  handleRepayment = () => {
    const { order } = this.props;
    if (!isEmpty(order.payment)) {
      const paymentAdditional = JSON.parse(
        head(order.payment.additional_information),
      );
    ...
    return (
      <React.Fragment>
        {get(order, 'payment.method') &&
          find(
            paymentUseRepayment,
            rePay => rePay === get(order, 'payment.method'),
          ) && (
            <Button
              className={s.repaymentBtn}
              outline
              onClick={this.handleRepayment}
            >
              {translate('repayment_now')}
            </Button>
          )}
    ...
    return (
      <Container className={s.root}>
        ...
        <Row className={cx({ [s.hidden]: this.state.showSummary })}>
                ...
                <Row>
                  <Col lg={12}>
                    <GuestOrderCompleteRegister
                      ...
                      errorMsg={get(registrationErrorCause, 'message')}
                    />
                  </Col>
                </Row>
              )}
          ...
          <section className={s.last}>
            <div className={cx(s.continue, s.mobileFooter)}>
              <Link className={s.linkContinueShopBtn} to="/">
                <Button className={s.continueShopBtn} outline>
                  {translate('continue_shopping')}{' '}
                  <IosArrowForward
                    icon="ios-arrow-forward"
